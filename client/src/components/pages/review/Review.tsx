...
const DownBtn = ({ fill }: { fill: string }) => {
  return (
    <svg
      width="9"
      height="12"
...
const UpDownWrapper = ({
  ...
  const [, setDownCnt] = useState<number>(0);
...
interface ReveiwProps extends ReviewInterface {
  review_id: string;
  hideUpDown: boolean;
}

const Review = (props: ReveiwProps) => {
  const {
    review_id,
    timestamp,
    walletAddress,
